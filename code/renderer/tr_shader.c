    ...
    //}
		...
		if ( ( pStage->stateBits & ( GLS_SRCBLEND_BITS | GLS_DSTBLEND_BITS ) ) &&
			...
			if ( ( blendSrcBits == GLS_SRCBLEND_SRC_ALPHA ) && ( blendDstBits == GLS_DSTBLEND_ONE_MINUS_SRC_ALPHA ) ) {
				...
				if(pStage->alphaGen == AGEN_IDENTITY) { //GUESSING
					pStage->alphaGen = AGEN_VERTEX;
				}

				if(pStage->rgbGen == CGEN_IDENTITY || pStage->rgbGen == CGEN_IDENTITY_LIGHTING) { //GUESSING
...
static void CreateInternalShaders( void ) {
	...
	Q_strncpyz( shader.name, "<mask>", sizeof( shader.name ) );
	stages[0].bundle[0].image[0] = tr.whiteImage;
	tr.maskShader = FinishShader();

	Q_strncpyz( shader.name, "<maskend>", sizeof( shader.name ) );
	tr.maskEndShader = FinishShader();
}
